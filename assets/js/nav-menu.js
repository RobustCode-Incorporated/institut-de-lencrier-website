(function () {
  ...
  function initHeaderEntrance() {
    ...
    // Only the home page carries the intro overlay; every other page plays
    // the header entrance immediately.
    var hasIntro = document.getElementById("intro-overlay");
    if (hasIntro) {
      window.addEventListener("ide:intro-complete", play, { once: true });
  ...
  function initNavMenu() {
    ...
    var mobileLinks = overlay.querySelectorAll(".mobile-link a");
    ...
    if (reduceMotion) {
      ...
      overlay.querySelectorAll(".mobile-link a").forEach(function (link) {
  ...
  function initLangSwitcher() {
    ...
    switches.forEach(function (switchEl) {
      ...
      if (hasGsap) {
        gsap.set(options, { opacity: 0, scale: 0.95, y: -8 });
      }
    ...
    function close(entry) {
      ...
      if (hasGsap) {
        gsap.to(entry.options, { opacity: 0, scale: 0.95, y: -8, duration: 0.2, ease: "power2.in" });
      } else {
